...
define('WSP_HOST','http://api.wx.aodianyun.com:8800/v3/');//wspapi请求地址
define('WSP_KEY','');//wspkey
class WspCurl
{
    public static function wspCurl($query_url, $data = '', $header = '', $method = 'GET', $wxUid ='' , $wxOpenId ='', $is_url = true, $timeout = 10)
    {
        if(empty($query_url)) {
            return 'Request URL can not be empty!';
        }
        $url = @parse_url(WSP_HOST.$query_url);
        if(empty($url)) {
            return 'Request URL can not be empty!';
        }
        ...
        if(!empty($wxUid) && !empty($wxOpenId)){
            $authorization = 'Authorization: wsp '.base64_encode('wspKey='.WSP_KEY.'&wxUid='.$wxUid.'&wxOpenId='.$wxOpenId);
        }
        else{
            $authorization = 'Authorization: wsp '.base64_encode('wspKey='.WSP_KEY);
        }
        if(empty($header)){
